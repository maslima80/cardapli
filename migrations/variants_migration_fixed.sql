...
-- We'll use a simpler approach to avoid the anonymous composite type error
DO $$
DECLARE
  product_rec RECORD;
  option_rec RECORD;
  ...
  value_index INTEGER;
BEGIN
  -- Loop through products with option_groups
  FOR product_rec IN 
  ...
  LOOP
    ...
    -- Loop through each option group using jsonb_array_elements with index
    FOR option_rec IN 
      SELECT value AS option_data, ordinality - 1 AS idx
      FROM jsonb_array_elements(product_rec.option_groups) WITH ORDINALITY
    LOOP
      -- Create option
      INSERT INTO product_options (product_id, user_id, name, sort)
      VALUES (product_rec.id, product_rec.user_id, option_rec.option_data->>'name', option_rec.idx)
      RETURNING id INTO option_id;
      
      -- Create option values if they exist
      IF option_rec.option_data->'values' IS NOT NULL AND jsonb_typeof(option_rec.option_data->'values') = 'array' THEN
        value_index := 0;
        
        -- Loop through each value using jsonb_array_elements with index
        FOR value_rec IN 
          SELECT value AS value_data, ordinality - 1 AS idx
          FROM jsonb_array_elements(option_rec.option_data->'values') WITH ORDINALITY
        LOOP
          ...
          VALUES (
            ...
            product_rec.user_id, 
            CASE 
              WHEN jsonb_typeof(value_rec.value_data) = 'string' THEN value_rec.value_data::text
              ELSE value_rec.value_data::text
            END,
            value_rec.idx
          );
